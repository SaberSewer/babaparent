...
//全选 
function checkAll(checked, myself){
	$("input[name=skuIds]").attr("checked",checked);
	if(myself.checked == true){
		$("#selectedCount").html($("input[name=skuIds]").length)
		getPrice()
	} else {
...
var product = {"id": "", "price": 0.0}

function checkOne(myself) {
	getPrice()
}

function getPrice() {
	var isCheck = $("input[name=skuIds]")
	var price = 0.0
	var ck = 0
	for (var i = 0; i < isCheck.length; i++){
		...
		price = parseFloat(price) + parseFloat($($(isCheck[i]).parent().parent().children(".p-price").children(".price")[0]).html()) * parseFloat($($(isCheck[i]).parent().parent().children(".p-quantity").children(".quantity-form").children(".quantity-text")[0]).val())
		ck++
	}
	$(".totalSkuPrice").html("￥" + price)
	$("#selectedCount").html(ck)
}
...
function delChecked(){
    ...
    for (var i = 0; i < isCheck.length; i++) {
        ...
        $(isCheck[i]).parent().parent().parent().remove()
        delCookies(isCheck[i].value)
    }
    getPrice()
}

function delCookies(skuId){
    //未登录只删除Cookies中的购物车数据
    var json = JSON.parse($.cookie("cart"))
    var skuIds =  json.skuId.split(",")
    var counts = json.count.split(",")
    ...
    json.skuId = skuIds.toString()
    json.count = counts.toString()
    $.cookie("cart", JSON.stringify(json), {path:"/", expires: 7, domain: "localhost", raw: false})
}

function AoD(x, myself) {
	switch (x) {
		case "-":
			$(myself).next().val(parseInt($(myself).next().val()) - 1)
			break;
		case "+":
			$(myself).prev().val(parseInt($(myself).prev().val()) + 1)
			break;
	}
	getPrice()
}
			...
			<div class="cart-inner">
				<div class="cart-thead clearfix">
					<div class="column t-checkbox form">
						<input type="checkbox" onclick="checkAll(checked, this)"></input> <label
							...
							<div class="item_form clearfix">
								<div class="cell p-checkbox">
									<input class="checkbox" type="checkbox" name="skuIds"
										value="${item.sku.id}" onclick="checkOne(this)">
								</div>
								...
								<div class="cell p-quantity" for-stock="for-stock-11345721">
									<div class="quantity-form">
										<a href="javascript:void(0);" class="decrement" onclick="AoD('-', this)">-</a> <input
											value="${item.conut }" type="text" class="quantity-text" onchange="getPrice()">
											<a href="javascript:void(0);" class="increment" onclick="AoD('+', this)">+</a>
									</div>
								</div>
				...
				<div class="ui-ceilinglamp-1" style="width: 988px; height: 49px;">
					<div class="cart-dibu ui-ceilinglamp-current"
						...
						<div class="control fdibu fdibucurrent">
						 <span class="delete"> 
						 	<b></b><a href="javascript:void(0);" onclick="delChecked()">删除选中的商品</a>
						</span> 
						<span class="shopping"> 
							<b></b><a href="/">继续购物</a>
						</span>
	...
	<div class="cart-empty">
		<div class="message">
			<ul>
				...
				<li>
					<a class="btn-1 login-btn mr10" href="http://localhost:8085/login">登录</a>
